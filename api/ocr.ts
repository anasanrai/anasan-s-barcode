...
function extractTextFromNimResponse(j: unknown): string {
  if (j && typeof j === "object" && "data" in j) {
    ...
    if (Array.isArray(data) && data[0]?.text_detections) {
      return data[0].text_detections.map((d) => d.text_prediction?.text ?? d.text ?? "").join("\n");
    }
  }
  if (j && typeof j === "object" && "choices" in j) {
    ...
    if (tc) {
      try { const p = JSON.parse(tc); if (Array.isArray(p)) return p.flat().join("\n"); } catch {}
      return String(tc);
    }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!image || typeof image !== "string") { res.status(400).json({ error: "missing image" }); return; }

  const b64 = image.replace(/^data:image\/\w+;base64,/, "");
  const dataUrl = `data:image/jpeg;base64,${b64}`;

  const nimKey = process.env.NIM_API_KEY;
  const nimUrl = process.env.NIM_OCR_URL || (nimKey ? "https://ai.api.nvidia.com/v1/cv/baidu/paddleocr" : null);
  ...
  if (nimKey && nimUrl) {
    try {
      ...
      if (r.ok) {
        ...
        if (text && text.trim()) { res.status(200).json({ text }); return; }
      }
    } catch {}
  }
